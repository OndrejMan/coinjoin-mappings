...
while (hasNext)
{
    ...
    try
    {
        isBlame = parser.IsBlame();
    }
    catch (Exception error)
    {
        errors++;
        transactions[txid] = new { status = "error", error = $"{error.GetType().Name}: {error.Message}" };
        Console.Error.WriteLine($"Sake transaction {txid} failed: {error.GetType().Name}: {error.Message}");
    ...
    if (!isBlame)
    {
        try
        {
            ...
            transactions[txid] = new { status = "complete", fee_rate_sat_per_byte = feeRate.SatoshiPerByte,
...
var completedTransactions = transactions.Count - errors;
var result = new { schema_version = "1.1", tool = "sake", seed, samples,
    summary = new { transactions = transactions.Count, completed = completedTransactions, errors,
        ...
        full_coinjoin_match_rate = completedTransactions == 0 ? null : (double?)perfectMatches / completedTransactions },
    transactions };
