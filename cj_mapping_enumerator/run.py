...
def parse_args():
    ...
    parser.add_argument(
        "--include-timing",
        action="store_true",
        help="Include measured durations; disabled by default so result JSON is reproducible.",
    )
    return parser.parse_args()


def enumeration_worker(inputs, outputs, mode, max_error, queue):
    """Importable multiprocessing target compatible with spawn and forkserver."""
    try:
...
def enumerate_once(inputs, outputs, mode, max_error, timeout, include_timing=False):
    try:
        duration, outcome = run_with_timeout(
            timeout, enumeration_worker, inputs, outputs, mode, max_error
        )
    ...
    if duration != inf and "error" in outcome:
        return {"timeout_seconds": timeout,
                "duration_seconds": duration if include_timing else None, "status": "error",
                "mapping_count": None, "error": outcome["error"]}
    count = inf if duration == inf else outcome["count"]
    return {"timeout_seconds": timeout,
            "duration_seconds": duration if include_timing and duration != inf else None,
            "status": "timeout" if duration == inf else "complete",
            "mapping_count": None if count == inf else count}


def enumerate_transaction(inputs, outputs, mode, max_error, timeout, retry_timeout, include_timing=False):
    attempts = [enumerate_once(inputs, outputs, mode, max_error, timeout, include_timing)]
    if attempts[0]["status"] == "timeout":
        attempts.append(enumerate_once(inputs, outputs, mode, max_error, retry_timeout, include_timing))
    final = attempts[-1]
...
def main():
    ...
    result = {"schema_version": ENUMERATOR_SCHEMA_VERSION, "tool": "coinjoin-mapping-enumerator",
              ...
              "summary": {"transactions": len(transactions), "completed": completed,
                          ...
                          "duration_seconds": time.time() - started if args.include_timing else None},
